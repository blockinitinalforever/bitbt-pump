...
const boot = async (fetchImpl: (input: string, init?: RequestInit) => Promise<unknown>, options: BootOptions = {}) => {
  ...
  const ethereum = { request: async ({ method }: { method: string }) => { providerCalls.push(method); if (method === "eth_accounts" || method === "eth_requestAccounts") return account ? [account] : []; if (method === "eth_chainId") return options.chainId ?? "0x38"; if (method === "wallet_switchEthereumChain" || method === "personal_sign") return method === "personal_sign" ? "0xsigned" : null; if (method === "eth_sendTransaction") return "0xlaunch"; if (method === "eth_getTransactionReceipt") return { status: "0x1" }; throw new Error(`unexpected provider call: ${method}`); }, on: (event: string, callback: (value: unknown) => void) => { providerEvents[event] = callback; } };
...
test("launch signing binds every prepare field and single-flights the wallet send", async () => {
  ...
  const prepared = { launch: { id: "launch-1", token_name: "Real Token", symbol: "REAL", creator_address: account, quote_token: "BNB" }, fee_wei: fee.fee_wei, factory_address: factory, fee_recipient: recipient, chain_id: "bsc", salt: `0x${"ab".repeat(32)}`, predicted_token_address: predicted, curve_address: curve, migration_threshold_wei: "6140000000000000000", quote_token_address: "0x0000000000000000000000000000000000000000", method: "launchTokenWithQuotePaid(string,string,uint256,bytes32,address)" };
  const run = async (mutate?: (value: typeof prepared) => typeof prepared) => {
    let sendCount = 0;
    const response = async (input: string) => {
      const url = String(input);
      ...
      if (url.includes("v1/token/prepare-launch")) return { ok: true, json: async () => ({ data: mutate ? mutate(prepared) : prepared }) };
      if (url.includes("v1/token/launch")) return { ok: true, json: async () => ({ data: { status: "deployed", contract_address: predicted } }) };
      throw new Error(`unmocked ${url}`);
    };
    const { window, providerCalls } = await boot(response, { account });
    ...
    return { sendCount, providerCalls };
  };
  assert.equal((await run()).sendCount, 1);
  const altered = [
  ...
  for (const [index, mutation] of altered.entries()) assert.equal((await run(mutation)).sendCount, 0, `altered launch field ${index} was signed`);
});
