...
test("discovery, full-market on-chain stream, rankings, and holders use real APIs with local filters", async () => {
  ...
  assert.match(alphaCard?.textContent || "", /TAX 6%/);
  assert.match(alphaCard?.textContent || "", /持有人6,062/);
  assert.match(alphaCard?.textContent || "", /还差 2 BNB/);
  assert.match(alphaCard?.textContent || "", /联合曲线 80%/);
  assert.equal(alphaCard?.querySelector(".market-signals"), null);
  for (const [filter, expected, count = 2] of [
    ["progress", "ALPHA"],
    ["gainers", "ALPHA"],
    ["volume", "ALPHA"],
    ["net-flow", "ALPHA"],
    ...
    window.document.querySelector(`[data-rank-filter="${filter}"]`)?.dispatchEvent(new window.Event("click", { bubbles: true }));
    const rows = window.document.querySelectorAll('[data-panel="rank"] .rank-row');
    assert.equal(rows.length, count, `${filter} rank did not render the expected local result count`);
    if (expected) assert.match(rows[0]?.textContent || "", new RegExp(expected));
  }
