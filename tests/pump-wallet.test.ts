...
test("mock EIP-1193 wallet signs, broadcasts and confirms a 0.05 BNB buy budget", async () => {
  ...
  const hash = await sendTransaction(wallet, { from: ADDRESS, to: ADDRESS, value: parseUnits("0.05"), gas: 300000n, maxPriorityFeePerGas: 1_000_000_000n, maxFeePerGas: 3_000_000_000n });
  ...
  assert.equal(transaction.maxPriorityFeePerGas, "0x3b9aca00");
  assert.equal(transaction.maxFeePerGas, "0xb2d05e00");
});
...
test("dynamic fee policy uses node priority and latest base fee", async () => {
  ...
  const fee = await getDynamicFeePolicy(wallet, 1_000_000_000n);
  assert.equal(fee.maxPriorityFeePerGas, 2_000_000_000n);
  assert.equal(fee.maxFeePerGas, 8_000_000_000n);
});
