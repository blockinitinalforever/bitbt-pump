...
(() => {
  ...
  const renderLiveRows = () => {
    ...
    const rows = visible.slice(0, 50).map((trade) => `<div class="live-row"><span class="trade-type ${String(trade.trade_type).toLowerCase() === "buy" ? "buy" : "sell"}">${escapeHtml(String(trade.trade_type || "TRADE").toUpperCase())}</span><div><p><b>${escapeHtml(short(trade.trader))}</b></p><small>${escapeHtml(decimal(trade.quote_amount || trade.bnb_amount))} ${escapeHtml(quote)} · ${escapeHtml(decimal(trade.token_amount))} ${escapeHtml(state.detail?.symbol || "TOKEN")}</small></div><small>${escapeHtml(age(new Date((trade.timestamp > 1e12 ? trade.timestamp : trade.timestamp * 1000)).toISOString()))}</small></div>`).join("");
