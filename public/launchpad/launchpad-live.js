...
(() => {
  ...
  if (!root) return;
  const state = { tokens: [], tokenFilter: "trending", tokenSearch: "", marketQuoteFilter: "all", marketCategoryFilter: "all", marketTypeFilter: "all", liveFilter: "all", rankFilter: "progress", rankWindow: "24h", myLaunchFilter: "all", historyFilter: "all", details: {}, holders: {}, comments: {}, alerts: [], points: null, referral: null, campaigns: [], kol: null, marketActivity: [], marketSummary: {}, config: null, selected: null, detail: null, trades: [], candles: [], migrationProof: null, myLaunches: [], history: [], walletHoldings: [], walletHoldingsComplete: false, tradeHistoryComplete: false, creatorRewards: [], favorites: [], side: "buy", quote: null, quoteKey: "", account: "", chainId: "", sessionExpiresAt: 0, provider: null, balances: { quote: null, token: null, gas: null }, chartInterval: 300, busy: false, launchBusy: false, launchQuote: "BNB", launchMode: "fair", curveMode: "standard", taxEnabled: false, launchLogoUrl: "", launchSnapshot: null, launchConfirmation: null, launchTerminal: false, liveRefreshPromise: null, tradesRefreshPromise: null };
  ...
  const safeExternalUrl = (value) => { const candidate = String(value || "").trim(); if (!/^https?:\/\//i.test(candidate)) return ""; try { const Url = window.URL; if (typeof Url !== "function") return candidate; const url = new Url(candidate); return ["http:", "https:"].includes(url.protocol) ? url.href : ""; } catch { return ""; } };
  const priceImpactLabel = (value) => { const impact = Number(value); if (!Number.isFinite(impact) || impact < 0) return "暂不可用"; if (impact > 0 && impact < .0001) return "<0.0001%"; return `${impact < 1 ? impact.toFixed(4) : impact.toFixed(2)}%`; };
  ...
  const usd = (value, compact = true) => {
    ...
    if (compact && absolute >= 1000) return new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", notation: "compact", maximumFractionDigits: 2 }).format(parsed);
    if (absolute < 0.000001) return `$${String(value).replace(/(\.\d*?[1-9])0+$/, "$1")}`;
    return parsed.toLocaleString("en-US", { style: "currency", currency: "USD", minimumFractionDigits: absolute >= 1 ? 2 : 0, maximumFractionDigits: absolute >= 1 ? 2 : 8 });
  };
  ...
  const renderRank = () => {
    ...
    let ranked = [...state.tokens]; if (state.rankFilter === "latest") ranked.sort((a, b) => tokenCreatedAt(b) - tokenCreatedAt(a)); else if (state.rankFilter === "migrated") ranked = ranked.filter(tokenIsMigrated).sort((a, b) => tokenCreatedAt(b) - tokenCreatedAt(a)); else if (state.rankFilter === "gainers") ranked.sort((a, b) => number(marketMetric(b, "price_change")) - number(marketMetric(a, "price_change"))); else if (state.rankFilter === "volume") ranked.sort((a, b) => number(marketMetric(b, "volume_usd") ?? marketMetric(b, "volume_quote")) - number(marketMetric(a, "volume_usd") ?? marketMetric(a, "volume_quote"))); else if (state.rankFilter === "net-flow") ranked.sort((a, b) => number(b.net_flow_usd_24h ?? b.net_flow_quote_24h) - number(a.net_flow_usd_24h ?? a.net_flow_quote_24h)); else if (state.rankFilter === "market-cap") ranked.sort((a, b) => number(b.market_cap_usd ?? b.market_cap_quote) - number(a.market_cap_usd ?? a.market_cap_quote)); else ranked.sort((a, b) => number(b.progress_percent) - number(a.progress_percent));
    const windowLabel = rankWindowLabel();
    const rows = ranked.slice(0, 20).map((token, index) => { const rawChange = marketMetric(token, "price_change"); const change = rawChange == null ? Number.NaN : Number(rawChange); const quote = String(token.quote_token || "BNB").toUpperCase(); const right = state.rankFilter === "volume" ? usdOrQuote(marketMetric(token, "volume_usd"), marketMetric(token, "volume_quote"), quote) : state.rankFilter === "net-flow" ? (hasNumber(token.net_flow_usd_24h) ? usd(token.net_flow_usd_24h) : `${decimal(token.net_flow_quote_24h)} ${quote}`) : state.rankFilter === "market-cap" ? usdOrQuote(token.market_cap_usd, token.market_cap_quote, quote) : usdOrQuote(token.current_price_usd, token.current_trade_price || token.current_price_quote || token.current_price_bnb, quote, false); const badge = Number.isFinite(change) ? `${change >= 0 ? "+" : ""}${change.toFixed(1)}%` : `${number(token.progress_percent).toFixed(0)}%`; return `<button class="rank-row" data-live-token="${escapeHtml(tokenAddress(token))}"><span class="num">${String(index + 1).padStart(2, "0")}</span><img src="${escapeHtml(assetImage(token))}" alt="${escapeHtml(token.symbol || "Token")}"><div><strong>${escapeHtml(token.symbol || token.token_name || "—")}</strong><small>${escapeHtml(status(token))} · ${escapeHtml(taxSummary(token))} · ${windowLabel} ${Number(marketMetric(token, "trade_count") || 0).toLocaleString("en-US")} 笔</small></div><div class="rank-price"><strong>${escapeHtml(right)}</strong><span class="${change < 0 ? "down" : "up"}">${escapeHtml(state.rankFilter === "net-flow" ? activitySummary(token) : badge)}</span></div></button>`; }).join("");
  ...
  const clearPrototype = () => {
    ...
    const projectSummary = $("[data-token-project-summary]"); if (projectSummary) projectSummary.hidden = true;
    const projectDescription = $("[data-token-description]"); if (projectDescription) { projectDescription.hidden = true; projectDescription.textContent = ""; }
  ...
  const invalidateQuote = () => { quoteRequestSequence += 1; state.quote = null; state.quoteKey = ""; text("[data-quote-output], [data-quote-min], [data-quote-route]", "—"); text("[data-quote-fee], [data-protocol-fee]", "等待报价"); text("[data-price-impact]", "等待报价"); };
  ...
  const walletPosition = (address, balanceRaw, token, detail) => {
    ...
    let indexedQuantity = 0; let averageCost = 0;
    rows.forEach((entry) => { const quantity = number(entry.token_amount); const quoteAmount = number(entry.quote_amount); if (!(quantity > 0) || !(quoteAmount >= 0)) return; if (String(entry.activity_type).toLowerCase() === "buy") { const next = indexedQuantity + quantity; averageCost = next > 0 ? (indexedQuantity * averageCost + quoteAmount) / next : 0; indexedQuantity = next; } else { indexedQuantity = Math.max(0, indexedQuantity - quantity); if (!indexedQuantity) averageCost = 0; } });
    ...
    if (!complete) return { actualQuantity, indexedQuantity, averageCost, value, cost: null, pnl: null, returnPercent: null, unit, quote, complete };
    return { actualQuantity, indexedQuantity, averageCost, value, cost, pnl, returnPercent, unit, quote, complete };
  };
  const renderWalletPosition = () => {
    ...
    const indexedPortfolioValue = state.walletHoldings.reduce((sum, holding) => { const address = String(holding.token_address || "").toLowerCase(); const item = state.tokens.find((token) => tokenAddress(token).toLowerCase() === address); const price = number(item?.current_price_usd); const quantity = number(formatUnits(BigInt(holding.balance_raw || "0"), 18, 18)); if (!(price > 0)) { portfolioComplete = false; return sum; } return sum + quantity * price; }, 0);
    ...
    const bar = $("[data-holding-bar]"); if (bar) bar.style.width = `${share || 0}%`;
  };
  ...
  const renderSelected = () => {
    ...
    const liquidityDisplay = detail.migrated ? state.migrationProof?.reserve_verified ? `${decimal(state.migrationProof.estimated_liquidity_quote)} ${quote}` : "待链上验证" : `${decimal(raised)} ${quote}`;
    ...
    const buyTax = number(detail.buy_tax_percent ?? token.buy_tax_percent); const sellTax = number(detail.sell_tax_percent ?? token.sell_tax_percent); const taxLabel = detail.tax_enabled || token.tax_enabled ? `买 ${buyTax}% / 卖 ${sellTax}%` : "0% / 0%"; renderProjectSummary(detail);
    text("[data-active-symbol]", detail.symbol || token.symbol); text("[data-active-quote]", quote); text("[data-active-status]", selectedStatus.toUpperCase()); text("[data-active-address]", `${short(address)} · BNB CHAIN`); text("[data-active-price-label]", hasNumber(token.current_price_usd) ? "PRICE / USD" : `PRICE / ${quote}`); text("[data-active-price]", priceDisplay); text("[data-active-market]", marketCapDisplay); text("[data-active-fdv]", fdvDisplay); text("[data-active-liquidity]", liquidityDisplay); text("[data-active-rank]", selectedStatus); text("[data-active-curve]", `${progress.toFixed(2)}%`); text("[data-active-volume]", volumeDisplay); text("[data-active-net-flow], [data-chain-net-flow]", netFlowDisplay); text("[data-active-buy-ratio]", buyRatioDisplay); text("[data-chain-buy-sell]", `${buyVolumeDisplay} / ${sellVolumeDisplay}`); text("[data-active-raised]", `${decimal(raised)} ${quote}`); text("[data-active-trade-count]", token.trade_count_24h ?? state.trades.length); text("[data-trade-count-label]", `共 ${state.trades.length} 笔`); text("[data-active-sold]", sold == null ? "—" : `${decimal(sold)} ${detail.symbol || token.symbol}`); text("[data-active-reserve]", detail.migrated ? state.migrationProof?.reserve_verified ? `DEX 流动性 ≈ ${decimal(state.migrationProof.estimated_liquidity_quote)} ${quote}` : "DEX 储备待链上验证" : `曲线储备 ${decimal(raised)} ${quote}`); text("[data-active-remaining]", detail.migrated ? state.migrationProof?.diagnostic === "verified" ? "PancakeSwap V2 迁移与储备已验证" : `迁移诊断：${state.migrationProof?.diagnostic || "读取中"}` : "迁移剩余由链上进度决定"); text("[data-chain-contract]", address); text("[data-chain-curve]", detail.curve_address || "—"); text("[data-chain-created]", formatDate(detail.submitted_at || token.submitted_at)); text("[data-chain-creator]", detail.creator || token.creator_address || "—"); text("[data-chain-status]", selectedStatus); text("[data-token-tax-detail], [data-token-tax]", taxLabel); text("[data-migration-router]", state.migrationProof?.router_verified ? `${short(state.migrationProof.router_address)} · 已验证` : state.migrationProof?.router_address ? `${short(state.migrationProof.router_address)} · 未验证` : "尚未读取"); text("[data-migration-pair]", state.migrationProof?.pair_address || (detail.migrated ? "迁移证明暂不可用" : "尚未迁移")); text("[data-migration-lp]", state.migrationProof?.lp_burn_verified ? `${formatUnits(BigInt(state.migrationProof.lp_burned_balance_raw || "0"), 18, 6)} LP · 已销毁` : detail.migrated ? "未验证" : "尚未迁移"); text("[data-holding-amount]", state.balances.token == null ? "—" : formatUnits(state.balances.token)); text("[data-holding-short]", state.balances.token == null ? "—" : formatUnits(state.balances.token, 18, 4)); renderWalletPosition(); text("[data-active-change]", Number.isFinite(Number(token.price_change_24h_percent)) ? `${Number(token.price_change_24h_percent) >= 0 ? "+" : ""}${Number(token.price_change_24h_percent).toFixed(2)}%` : "—"); $$('[data-active-image]').forEach((node) => { node.src = assetImage({ ...token, ...detail }); }); $$('[data-active-curve-bar]').forEach((node) => { node.style.width = `${progress}%`; }); $$('[data-copy-token-address]').forEach((node) => { node.dataset.tokenAddress = address; node.title = `复制完整地址 ${address}`; }); const favorite = state.favorites.some((item) => String(item.contract_address || "").toLowerCase() === address.toLowerCase()); $$('[data-favorite-token]').forEach((node) => { node.classList.toggle("active", favorite); node.title = favorite ? "取消自选" : "加入自选"; }); const holderPayload = state.holders[address.toLowerCase()]; if (holderPayload) renderHolders(address, holderPayload); else { text("[data-holder-count]", "点击持有人页加载"); const holderList = $("[data-holder-list]"); if (holderList) holderList.innerHTML = `<p class="footer-note">打开本页后按需读取真实持有人数据。</p>`; }
