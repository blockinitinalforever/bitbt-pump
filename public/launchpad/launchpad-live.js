...
(() => {
  ...
  const setTaxMode = (enabled, preset = false) => { state.taxEnabled = Boolean(enabled); $$('[data-tax-mode]').forEach((choice) => choice.classList.toggle("active", (choice.dataset.taxMode === "tax") === state.taxEnabled)); $$('[data-tax-fields]').forEach((group) => { group.hidden = !state.taxEnabled; }); if (state.taxEnabled && preset) { const defaults = { "#buy-tax-rate": "3", "#sell-tax-rate": "5", "#funds-recipient-pct": "40", "#burn-pct": "20", "#holders-pct": "20", "#liquidity-pct": "20", "#min-dividend-balance": "100000", "#tax-recipient-wallet": state.account || "" }; Object.entries(defaults).forEach(([selector, value]) => { const input = $(selector); if (input && !input.value) input.value = value; }); } invalidateLaunchSnapshot(); };
  const applyLaunchMode = (mode) => { state.launchMode = ["custom", "community"].includes(mode) ? mode : "fair"; setCurveMode(state.launchMode === "custom" ? "custom" : "standard"); setTaxMode(state.launchMode === "community", state.launchMode === "community"); };
  ...
  const bindNavigation = () => { $$("[data-open]").forEach((node) => node.addEventListener("click", (event) => { if (node.dataset.liveToken) return; event.preventDefault(); if (node.dataset.launchMode) applyLaunchMode(node.dataset.launchMode); show(node.dataset.open); if (node.dataset.side) applySide(node.dataset.side === "sell"); })); $$("[data-nav]").forEach((node) => node.addEventListener("click", () => show(node.dataset.nav))); $$("[data-detail-tab]").forEach((node) => node.addEventListener("click", () => { $$("[data-detail-tab]").forEach((tab) => tab.classList.toggle("active", tab === node)); $$("[data-detail-panel]").forEach((panel) => panel.classList.toggle("active", panel.dataset.detailPanel === node.dataset.detailTab)); })); $$("[data-amount]").forEach((node) => node.addEventListener("click", () => { const input = $("#trade-amount"); if (input) input.value = node.dataset.amount === "MAX" ? "" : node.dataset.amount; invalidateQuote(); })); $$("[data-lang-toggle]").forEach((node) => node.addEventListener("click", () => { const target = pumpLocale() === "zh" ? "en" : "zh"; location.assign(location.pathname.replace(/^\/(en|zh)(?=\/|$)/, `/${target}`) + location.search); })); };
