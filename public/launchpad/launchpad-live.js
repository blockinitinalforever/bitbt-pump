...
(() => {
  ...
  setLaunchAvailability(false); invalidateLaunchSnapshot(); clearPrototype(); bindProviderEvents(); bindNavigation(); bind(); void load(); window.setInterval(() => { Promise.all([api("v1/pump/tokens"), loadAllDetails()]).then(([tokens]) => { state.tokens = tokens; renderTokens(); }).catch((error) => renderUnavailable(error)); if (state.selected) loadDetail(state.selected).catch((error) => renderUnavailable(error)); }, 15000);
})();
