...
(() => {
  ...
  if (!root) return;
  const state = { tokens: [], tokenFilter: "trending", tokenSearch: "", liveFilter: "all", rankFilter: "progress", myLaunchFilter: "all", historyFilter: "all", details: {}, holders: {}, marketActivity: [], marketSummary: {}, config: null, selected: null, detail: null, trades: [], myLaunches: [], history: [], creatorRewards: [], favorites: [], side: "buy", quote: null, quoteKey: "", account: "", chainId: "", sessionExpiresAt: 0, provider: null, balances: { quote: null, token: null, gas: null }, chartInterval: 300, busy: false, launchBusy: false, launchQuote: "BNB", launchMode: "fair", curveMode: "standard", taxEnabled: false, launchLogoUrl: "", launchSnapshot: null, launchConfirmation: null, launchTerminal: false, liveRefreshPromise: null, tradesRefreshPromise: null };
  ...
  const resetProviderState = (message = "钱包状态已变化，请重新连接") => { sessionStorage.removeItem(SESSION_KEY); sessionStorage.removeItem(SESSION_ADDRESS_KEY); userDataRequestSequence += 1; balanceRequestSequence += 1; state.account = ""; state.chainId = ""; state.sessionExpiresAt = 0; state.balances = { quote: null, token: null, gas: null }; state.myLaunches = []; state.history = []; state.creatorRewards = []; setLaunchAvailability(false); invalidateQuote(); invalidateLaunchSnapshot(); renderMyPanels(); $$('[data-wallet-label], .connect-global, .connect').forEach((node) => { node.textContent = "连接钱包"; }); if (message) toast(message); };
  ...
  const loadDetail = async (token, { refreshBalance = true, forceDetail = false, refreshTrades = true } = {}) => {
    ...
    const requestSequence = ++detailRequestSequence; state.selected = token; const address = tokenAddress(token).toLowerCase(); const [detail, trades] = await Promise.all([!forceDetail && state.details[address] ? Promise.resolve(state.details[address]) : api(`v1/pump/detail?address=${encodeURIComponent(tokenAddress(token))}`), refreshTrades ? api(`v1/pump/trades?token_address=${encodeURIComponent(tokenAddress(token))}`) : Promise.resolve(state.trades)]); if (requestSequence !== detailRequestSequence || tokenAddress(state.selected).toLowerCase() !== address) return false; state.detail = detail; state.details[address] = detail; state.trades = trades; selectedDetailAddress = address; if (refreshBalance) state.balances = { quote: null, token: null, gas: null }; renderSelected(); renderTradeConfig(); renderLiveRows(); drawCharts(); if (state.account && refreshBalance) await refreshBalances(); return true;
  };
  ...
  const openToken = async (token, { historyMode = "push", fallbackDetail = null } = {}) => {
    try { if (await loadDetail(token) === false) return; }
      ...
      const address = tokenAddress(token).toLowerCase(); if (tokenAddress(state.selected).toLowerCase() !== address) return; state.selected = token; state.detail = fallbackDetail; state.details[address] = fallbackDetail; state.trades = []; selectedDetailAddress = address; renderSelected(); renderTradeConfig(); renderLiveRows(); drawCharts();
    }
    activateDetail();
  ...
  const renderUnavailable = (error) => { clearPrototype(); state.tokens = []; state.details = {}; state.selected = null; state.detail = null; state.trades = []; selectedDetailAddress = ""; state.marketActivity = []; state.marketSummary = {}; const banner = $("[data-api-status]"); if (banner) banner.textContent = "实时 Pump 数据暂不可用"; text("[data-market-stream-status]", "BNB Chain 数据流暂不可用"); toastError(error, "实时 Pump 数据暂不可用，请稍后重试"); };
  ...
  const refreshSelectedTrades = () => { if (state.tradesRefreshPromise || !state.selected) return state.tradesRefreshPromise; const address = tokenAddress(state.selected).toLowerCase(); if (selectedDetailAddress !== address) return null; state.tradesRefreshPromise = (async () => { const trades = await api(`v1/pump/trades?token_address=${encodeURIComponent(address)}`); if (tokenAddress(state.selected).toLowerCase() !== address || selectedDetailAddress !== address) return; state.trades = trades; renderSelected(); drawCharts(); })().catch((error) => toastError(error, "成交记录刷新失败，请稍后重试")).finally(() => { state.tradesRefreshPromise = null; }); return state.tradesRefreshPromise; };
  const refreshLive = () => { if (state.liveRefreshPromise) return state.liveRefreshPromise; state.liveRefreshPromise = (async () => { const [tokens, market] = await Promise.all([api("v1/pump/tokens"), api("v1/pump/market-activity?limit=100").catch(() => null)]); state.tokens = tokens; if (market) applyMarketActivity(market); else text("[data-market-stream-status]", "全市场链上动态暂不可用"); renderTokens(); if (state.selected) { const address = tokenAddress(state.selected).toLowerCase(); const freshToken = state.tokens.find((token) => tokenAddress(token).toLowerCase() === address) || state.selected; await loadDetail(freshToken, { refreshBalance: false, forceDetail: true, refreshTrades: false }).catch((error) => toastError(error, "项目详情刷新失败，请稍后重试")); } })().catch((error) => renderUnavailable(error)).finally(() => { state.liveRefreshPromise = null; }); return state.liveRefreshPromise; };
