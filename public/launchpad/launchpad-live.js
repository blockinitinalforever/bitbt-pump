...
(() => {
  ...
  const renderRank = () => {
    ...
    let ranked = [...state.tokens]; if (state.rankFilter === "latest") ranked.sort((a, b) => tokenCreatedAt(b) - tokenCreatedAt(a)); else if (state.rankFilter === "migrated") ranked = ranked.filter(tokenIsMigrated).sort((a, b) => tokenCreatedAt(b) - tokenCreatedAt(a)); else if (state.rankFilter === "gainers") ranked.sort((a, b) => number(b.price_change_24h_percent) - number(a.price_change_24h_percent)); else if (state.rankFilter === "volume") ranked.sort((a, b) => number(b.volume_quote_24h) - number(a.volume_quote_24h)); else if (state.rankFilter === "market-cap") ranked.sort((a, b) => number(b.market_cap_quote) - number(a.market_cap_quote)); else ranked.sort((a, b) => number(b.progress_percent) - number(a.progress_percent));
    const rows = ranked.slice(0, 20).map((token, index) => { const change = Number(token.price_change_24h_percent); const right = state.rankFilter === "volume" ? `${decimal(token.volume_quote_24h)} ${token.quote_token || "BNB"}` : state.rankFilter === "market-cap" ? `${decimal(token.market_cap_quote)} ${token.quote_token || "BNB"}` : decimal(token.current_trade_price || token.current_price_quote || token.current_price_bnb); const badge = Number.isFinite(change) ? `${change >= 0 ? "+" : ""}${change.toFixed(1)}%` : `${number(token.progress_percent).toFixed(0)}%`; return `<button class="rank-row" data-live-token="${escapeHtml(tokenAddress(token))}"><span class="num">${String(index + 1).padStart(2, "0")}</span><img src="${escapeHtml(assetImage(token))}" alt="${escapeHtml(token.symbol || "Token")}"><div><strong>${escapeHtml(token.symbol || token.token_name || "—")}</strong><small>${escapeHtml(status(token))} · 24H ${Number(token.trade_count_24h || 0).toLocaleString("en-US")} 笔</small></div><div class="rank-price"><strong>${escapeHtml(right)}</strong><span class="${change < 0 ? "down" : "up"}">${escapeHtml(badge)}</span></div></button>`; }).join("");
