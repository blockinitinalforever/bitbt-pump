...
(() => {
  ...
  if (!root) return;
  window.setTimeout(() => $$('[data-panel="detail"] .actions [data-open="watchlist"]').forEach((node) => node.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); toggleFavorite().catch((error) => toastError(error)); }, true)), 0);
  ...
  const pumpLocale = () => location?.pathname?.match(/^\/(en|zh)(?:\/|$)/)?.[1] || "en";
  const pumpBasePath = () => `/${pumpLocale()}/pump`;
  const routeTokenAddress = () => {
    const match = location?.pathname?.match(/^\/(?:en|zh)\/pump\/(0x[0-9a-fA-F]{40})\/?$/);
  ...
  const copyTokenAddress = async () => {
    ...
    if (navigator?.clipboard?.writeText) await navigator.clipboard.writeText(address);
      ...
      const input = document.createElement("textarea"); input.value = address; input.setAttribute("readonly", ""); input.style.position = "fixed"; input.style.opacity = "0"; document.body.appendChild(input); input.select(); const copied = document.execCommand?.("copy"); input.remove(); if (!copied) throw new Error("浏览器禁止复制，请长按地址手动复制");
    }
    toast("完整代币地址已复制");
  };
  ...
  const clearPrototype = () => {
    ...
    ["[data-panel='discover'] .market-strip", ".pulse-line", "[data-panel='live'] .live-row", "[data-panel='rank'] .rank-row", "[data-panel='activity'] .activity-card", "[data-panel='announcements'] .announcement-card", "[data-panel='announcements'] .announcement-detail", "[data-panel='detail'] [data-detail-panel='trades'] .live-row", "[data-panel='detail'] [data-detail-panel='holders'] .data-table", "[data-panel='detail'] [data-detail-panel='data'] .data-table", "[data-panel='success'] .launch-card", "[data-panel='success'] .review-block", "[data-panel='create-review'] .review-block", "[data-panel='my-launches'] .summary-hero", "[data-panel='my-launches'] .launch-card", "[data-panel='profile'] .profile-card", "[data-panel='watchlist'] .token-card"].forEach((selector) => $$(selector).forEach((node) => node.remove()));
  ...
  const renderWalletState = () => { $$('[data-wallet-label], .connect-global, .connect').forEach((node) => { node.textContent = state.account ? short(state.account) : "连接钱包"; }); };
  const restoreSession = async () => {
    ...
    try {
      ...
      const chainId = window.ethereum ? await window.ethereum.request({ method: "eth_chainId" }) : "";
  ...
  const bindProviderEvents = () => { if (window.ethereum?.on) { window.ethereum.on("accountsChanged", (accounts) => { const next = String(accounts?.[0] || "").toLowerCase(); if (!next) { window.setTimeout(() => { void restoreSession(); }, 350); return; } if (state.account && next === state.account) return; resetProviderState(); }); window.ethereum.on("chainChanged", () => resetProviderState("网络已变化，请重新连接 BSC")); } window.addEventListener("focus", () => { void revalidateSession(); }); document.addEventListener("visibilitychange", () => { if (document.visibilityState === "visible") void revalidateSession(); }); };
  ...
  const drawCharts = () => {
    ...
    if (!state.trades.length) { ["#launch-kline", "#trade-kline"].forEach((selector) => { const entry = charts.get(selector); entry?.chart.remove?.(); charts.delete(selector); $(selector)?.replaceChildren(); }); return; }
    const ratioDecimal = (quoteRaw, tokenRaw) => { try { const quote = BigInt(String(quoteRaw || "0")); const token = BigInt(String(tokenRaw || "0")); if (quote <= 0n || token <= 0n) return "0"; const scale = 10n ** 18n; const scaled = (quote * scale) / token; const whole = scaled / scale; const fraction = (scaled % scale).toString().padStart(18, "0").replace(/0+$/, "").slice(0, 15); return fraction ? `${whole}.${fraction}` : whole.toString(); } catch { return "0"; } };
    // Lightweight Charts accepts numbers only. The exact integer ratio is bounded to 15 decimal places before this final chart-API conversion.
    const chartPrice = (quoteRaw, tokenRaw) => { const decimal = ratioDecimal(quoteRaw, tokenRaw); const value = parseFloat(decimal); return Number.isFinite(value) ? value : 0; }; const prices = state.trades.map((trade) => { const quote = trade.quote_amount || trade.bnb_amount; const token = trade.token_amount; return { time: Math.floor((trade.timestamp > 1e12 ? trade.timestamp : trade.timestamp * 1000) / 300000) * 300, price: chartPrice(quote, token) }; }).filter((point) => point.price > 0).sort((a, b) => a.time - b.time);
  ...
  const applySide = (sell) => { state.side = sell ? "sell" : "buy"; $$('[data-trade-side]').forEach((node) => node.classList.toggle("active", (node.dataset.tradeSide === "sell") === sell)); const token = state.selected; const detail = state.detail; if (!token || !detail) return; const balance = sell ? state.balances.token : state.balances.quote; text("[data-order-label]", sell ? "卖出数量" : "支付"); text("[data-order-unit]", sell ? detail.symbol : detail.quote_token); text("[data-order-balance]", balance == null ? "钱包余额 —" : `钱包余额 ${formatUnits(balance)} ${sell ? detail.symbol : detail.quote_token}`); const submit = $("#trade-submit"); if (submit) { submit.textContent = sell ? "连接钱包并卖出" : "连接钱包并买入"; submit.classList.toggle("red", sell); } updateQuote().catch((error) => toastError(error, "报价获取失败，请稍后重试")); };
  ...
  const launchPreflight = async ({ from, to, data, value }) => {
    ...
    let estimatedGas;
    try {
      estimatedGas = BigInt(await window.ethereum.request({ method: "eth_estimateGas", params: [estimateTx] }));
    } catch (error) {
  ...
  const bindLiveTokenSelection = () => $$("[data-live-token]").forEach((node) => node.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); const token = state.tokens.find((item) => tokenAddress(item).toLowerCase() === String(node.dataset.liveToken || "").toLowerCase()); if (token) openToken(token).catch((error) => toastError(error, "代币详情加载失败，请稍后重试")); }, true));
  const show = (name) => { const target = root.querySelector(`[data-panel="${CSS.escape(name)}"]`); if (!target) return; invalidateQuote(); if (name === "create-mode" || (name === "create-basic" && state.launchTerminal)) resetLaunchFlow(); else if (!state.launchSnapshot || name !== "create-review") invalidateLaunchSnapshot(); $$(`[data-panel]`).forEach((panel) => panel.classList.toggle("active", panel === target)); target.classList.add("has-bottom-nav"); target.scrollTop = 0; history?.replaceState?.(null, "", `${pumpBasePath()}?screen=${encodeURIComponent(name)}`); };
  const bindNavigation = () => { $$("[data-open]").forEach((node) => node.addEventListener("click", (event) => { if (node.dataset.liveToken) return; event.preventDefault(); show(node.dataset.open); if (node.dataset.side) applySide(node.dataset.side === "sell"); })); $$("[data-nav]").forEach((node) => node.addEventListener("click", () => show(node.dataset.nav))); $$("[data-detail-tab]").forEach((node) => node.addEventListener("click", () => $$("[data-detail-panel]").forEach((panel) => panel.classList.toggle("active", panel.dataset.detailPanel === node.dataset.detailTab)))); $$("[data-amount]").forEach((node) => node.addEventListener("click", () => { const input = $("#trade-amount"); if (input) input.value = node.dataset.amount === "MAX" ? "" : node.dataset.amount; invalidateQuote(); })); $$("[data-lang-toggle]").forEach((node) => node.addEventListener("click", () => toast("语言切换将在真实数据加载后应用"))); };
  const bind = () => { $$('[data-wallet-label], .connect-global, .connect').forEach((node) => { node.dataset.walletBound = "1"; node.addEventListener("click", () => connectWallet().catch((error) => toastError(error, "钱包连接失败，请重试"))); }); $$('[data-copy-token-address]').forEach((node) => node.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); copyTokenAddress().catch((error) => toastError(error, "代币地址复制失败")); }, true)); $$("[data-trade-side]").forEach((node) => node.addEventListener("click", () => applySide(node.dataset.tradeSide === "sell"))); const input = $("#trade-amount"); input?.addEventListener("input", () => { window.clearTimeout(input._quoteTimer); input._quoteTimer = window.setTimeout(() => updateQuote().catch((error) => toastError(error, "报价获取失败，请稍后重试")), 350); }); $("#trade-submit")?.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); executeTrade().catch((error) => toastError(error, "交易提交失败，请稍后重试")); }, true); $$('[data-launch-load], [data-launch-publish]').forEach((node) => { if (node.dataset.launchBound) return; node.dataset.launchBound = "1"; node.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); launchToken().catch((error) => toastError(error, "发币提交失败，请稍后重试")); }, true); }); };
  ...
  const renderUnavailable = (error) => { clearPrototype(); state.tokens = []; state.details = {}; state.selected = null; state.detail = null; state.trades = []; const banner = $("[data-api-status]"); if (banner) banner.textContent = "实时 Pump 数据暂不可用"; toastError(error, "实时 Pump 数据暂不可用，请稍后重试"); };
  const load = async () => { try { const [tokens, config] = await Promise.all([api("v1/pump/tokens"), api("v1/app/config").catch(() => null)]); state.tokens = tokens; state.config = config; await loadAllDetails(); renderTokens(); const routedAddress = routeTokenAddress(); if (routedAddress) await openTokenAddress(routedAddress, { historyMode: "replace" }); else { const first = state.tokens.find((token) => tokenAddress(token)); if (first) await loadDetail(first); } } catch (error) { renderUnavailable(error); } };
  const refreshSelectedTrades = () => { if (state.refreshPromise || !state.selected) return state.refreshPromise; state.refreshPromise = (async () => { state.trades = await api(`v1/pump/trades?token_address=${encodeURIComponent(tokenAddress(state.selected))}`); renderSelected(); renderLiveRows(); drawCharts(); })().catch((error) => toastError(error, "成交记录刷新失败，请稍后重试")).finally(() => { state.refreshPromise = null; }); return state.refreshPromise; };
  const refreshLive = () => { if (state.refreshPromise) return state.refreshPromise; state.refreshPromise = (async () => { const [tokens] = await Promise.all([api("v1/pump/tokens"), loadAllDetails()]); state.tokens = tokens; renderTokens(); if (state.selected) await loadDetail(state.selected, { refreshBalance: false }); })().catch((error) => renderUnavailable(error)).finally(() => { state.refreshPromise = null; }); return state.refreshPromise; };
