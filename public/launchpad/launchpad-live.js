...
(() => {
  ...
  if (!root) return;
  const state = { tokens: [], tokenFilter: "trending", tokenSearch: "", liveFilter: "all", rankFilter: "progress", myLaunchFilter: "all", historyFilter: "all", details: {}, config: null, selected: null, detail: null, trades: [], myLaunches: [], history: [], favorites: [], side: "buy", quote: null, quoteKey: "", account: "", chainId: "", sessionExpiresAt: 0, balances: { quote: null, token: null, gas: null }, chartInterval: 300, busy: false, launchBusy: false, launchQuote: "BNB", taxEnabled: false, launchLogoUrl: "", launchSnapshot: null, launchTerminal: false, refreshPromise: null };
  ...
  const shareToken = async () => { const address = tokenAddress(state.selected); if (!/^0x[0-9a-fA-F]{40}$/.test(address)) throw new Error("当前代币链接无效"); const url = `${location.origin || "https://bitbt.fun"}${pumpBasePath()}/${address.toLowerCase()}`; await copyText(url); toast("代币详情链接已复制"); };
  ...
  const renderLiveRows = () => {
    const quote = state.detail?.quote_token || "BNB"; const visible = state.trades.filter((trade) => state.liveFilter === "all" || String(trade.trade_type).toLowerCase() === state.liveFilter);
    const rows = visible.slice(0, 50).map((trade) => `<div class="live-row"><span class="trade-type ${String(trade.trade_type).toLowerCase() === "buy" ? "buy" : "sell"}">${String(trade.trade_type || "TRADE").toUpperCase()}</span><div><p><b>${short(trade.trader)}</b></p><small>${decimal(trade.quote_amount || trade.bnb_amount)} ${quote} · ${decimal(trade.token_amount)} ${state.detail?.symbol || "TOKEN"}</small></div><small>${age(new Date((trade.timestamp > 1e12 ? trade.timestamp : trade.timestamp * 1000)).toISOString())}</small></div>`).join("");
  ...
  const renderRank = () => {
    ...
    const rows = ranked.slice(0, 10).map((token, index) => `<button class="rank-row" data-live-token="${tokenAddress(token)}"><span class="num">${String(index + 1).padStart(2, "0")}</span><img src="${assetImage(token)}" alt="${token.symbol || "Token"}"><div><strong>${token.symbol || token.token_name || "—"}</strong><small>${status(token)} · ${token.quote_token || "BNB"}</small></div><div class="rank-price"><strong>${decimal(token.current_price_quote || token.current_price_bnb)}</strong><span class="up">${number(token.progress_percent).toFixed(0)}%</span></div></button>`).join("");
  ...
  const renderMyPanels = () => {
    ...
    if (launchPanel) {
      ...
      const cards = visibleLaunches.map((launch) => `<button class="launch-card" data-live-token="${launch.contract_address || ""}"><div class="launch-card-head"><img src="${assetImage(launch)}" alt="${launch.token_name || launch.symbol || "Token"}"><div><strong>${launch.token_name || launch.symbol || "—"} · ${launch.symbol || "—"}</strong><small>${status(launch)} · ${age(launch.submitted_at)}</small></div><span class="tag lime">${String(launch.status || "").toUpperCase()}</span></div><div class="card-metrics"><div><span>计价</span><strong>${launch.quote_token || "BNB"}</strong></div><div><span>地址</span><strong>${short(launch.contract_address)}</strong></div><div><span>网络</span><strong>${launch.chain_id || "bsc"}</strong></div></div></button>`).join("");
  ...
  const invalidateLaunchSnapshot = () => { if (state.launchBusy) return; state.launchSnapshot = null; clearLaunchReview(); text("[data-launch-review-tax]", "—"); };
  ...
  const renderWalletState = () => { $$('[data-wallet-label], .connect-global, .connect').forEach((node) => { node.textContent = state.account ? short(state.account) : "连接钱包"; }); if (state.detail) applySide(state.side === "sell"); };
  ...
  const launchTokenSingleFlight = async () => {
    ...
    if (!snapshot || snapshot.key !== snapshotKey) {
      ...
      const launchSettings = tax ? { antisniper: true, enable_tax: true, request_platform_lp: false, buy_tax_rate: tax.buy_tax_rate, sell_tax_rate: tax.sell_tax_rate, funds_recipient_pct: tax.funds_recipient_pct, burn_pct: tax.burn_pct, holders_pct: tax.holders_pct, liquidity_pct: tax.liquidity_pct, min_dividend_balance: tax.min_dividend_balance, recipient_wallet: tax.recipient } : { antisniper: true, enable_tax: false, request_platform_lp: false };
    ...
    const launchData = encodeLaunch(prepared, tax); const launchValue = BigInt(fee.fee_wei); const preflight = await launchPreflight({ from: address, to: prepared.factory_address, data: launchData, value: launchValue });
  ...
  const bindNavigation = () => { $$("[data-open]").forEach((node) => node.addEventListener("click", (event) => { if (node.dataset.liveToken) return; event.preventDefault(); show(node.dataset.open); if (node.dataset.side) applySide(node.dataset.side === "sell"); })); $$("[data-nav]").forEach((node) => node.addEventListener("click", () => show(node.dataset.nav))); $$("[data-detail-tab]").forEach((node) => node.addEventListener("click", () => { $$("[data-detail-tab]").forEach((tab) => tab.classList.toggle("active", tab === node)); $$("[data-detail-panel]").forEach((panel) => panel.classList.toggle("active", panel.dataset.detailPanel === node.dataset.detailTab)); })); $$("[data-amount]").forEach((node) => node.addEventListener("click", () => { const input = $("#trade-amount"); if (input) input.value = node.dataset.amount === "MAX" ? "" : node.dataset.amount; invalidateQuote(); })); $$("[data-lang-toggle]").forEach((node) => node.addEventListener("click", () => { const target = pumpLocale() === "zh" ? "en" : "zh"; location.assign(location.pathname.replace(/^\/(en|zh)(?=\/|$)/, `/${target}`) + location.search); })); };
  const bind = () => { $$('[data-wallet-label], .connect-global, .connect').forEach((node) => { node.dataset.walletBound = "1"; node.addEventListener("click", () => connectWallet().catch((error) => toastError(error, "钱包连接失败，请重试"))); }); $$('[data-copy-token-address]').forEach((node) => node.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); copyTokenAddress().catch((error) => toastError(error, "代币地址复制失败")); }, true)); $$('[data-share-token]').forEach((node) => node.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); shareToken().catch((error) => toastError(error, "代币链接复制失败")); }, true)); $$('[data-favorite-token]').forEach((node) => node.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); toggleFavorite().catch((error) => toastError(error, "自选更新失败")); }, true)); $$('[data-chart-interval]').forEach((node) => node.addEventListener("click", () => { const interval = Number(node.dataset.chartInterval); if (![60, 300, 900, 3600, 14400, 86400].includes(interval)) return; state.chartInterval = interval; $$('[data-chart-interval]').forEach((choice) => choice.classList.toggle("active", Number(choice.dataset.chartInterval) === interval)); drawCharts(); })); $$("[data-trade-side]").forEach((node) => node.addEventListener("click", () => applySide(node.dataset.tradeSide === "sell"))); const input = $("#trade-amount"); input?.addEventListener("input", () => { window.clearTimeout(input._quoteTimer); input._quoteTimer = window.setTimeout(() => updateQuote().catch((error) => toastError(error, "报价获取失败，请稍后重试")), 350); }); $("#trade-submit")?.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); executeTrade().catch((error) => toastError(error, "交易提交失败，请稍后重试")); }, true); $$('[data-launch-load], [data-launch-publish]').forEach((node) => { if (node.dataset.launchBound) return; node.dataset.launchBound = "1"; node.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); launchToken().catch((error) => toastError(error, "发币提交失败，请稍后重试")); }, true); }); };
  ...
  const load = async () => { try { const [tokens, config] = await Promise.all([api("v1/pump/tokens"), api("v1/app/config").catch(() => null)]); state.tokens = tokens; state.config = config; renderTokens(); const routedAddress = routeTokenAddress(); if (routedAddress) await openTokenAddress(routedAddress, { historyMode: "replace" }); else { const first = state.tokens.find((token) => tokenAddress(token)); if (first) void loadDetail(first).catch((error) => toastError(error, "首个项目详情加载失败")); } } catch (error) { renderUnavailable(error); } };
  ...
  const refreshLive = () => { if (state.refreshPromise) return state.refreshPromise; state.refreshPromise = (async () => { state.tokens = await api("v1/pump/tokens"); renderTokens(); if (state.selected) await loadDetail(state.selected, { refreshBalance: false }); })().catch((error) => renderUnavailable(error)).finally(() => { state.refreshPromise = null; }); return state.refreshPromise; };
  ...
  $$('[data-tax-mode]').forEach((node) => node.addEventListener("click", () => {
    state.taxEnabled = node.dataset.taxMode === "tax";
    $$('[data-tax-mode]').forEach((choice) => choice.classList.toggle("active", choice === node));
    $$('[data-tax-fields]').forEach((group) => { group.hidden = !state.taxEnabled; });
  ...
  window.addEventListener("popstate", () => { const address = routeTokenAddress(); if (address) { void openTokenAddress(address, { historyMode: null }).catch((error) => toastError(error, "代币详情恢复失败")); return; } const discover = $('[data-panel="discover"]'); if (discover) { $$('[data-panel]').forEach((panel) => panel.classList.toggle("active", panel === discover)); discover.classList.add("has-bottom-nav"); discover.scrollTop = 0; } });
  let refreshCycle = 0;
