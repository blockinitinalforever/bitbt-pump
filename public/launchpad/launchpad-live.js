...
(() => {
  ...
  const show = (name) => { const target = root.querySelector(`[data-panel="${CSS.escape(name)}"]`); if (!target) return; invalidateQuote(); if (name === "create-mode" || (name === "create-basic" && state.launchTerminal)) resetLaunchFlow(); else if (!state.launchSnapshot || name !== "create-review") invalidateLaunchSnapshot(); $$(`[data-panel]`).forEach((panel) => panel.classList.toggle("active", panel === target)); target.classList.add("has-bottom-nav"); target.scrollTop = 0; routeHistory()?.replaceState?.(null, "", `${pumpBasePath()}?screen=${encodeURIComponent(name)}`); };
  const bindNavigation = () => { $$("[data-open]").forEach((node) => node.addEventListener("click", (event) => { if (node.dataset.liveToken) return; event.preventDefault(); if (node.dataset.launchMode) applyLaunchMode(node.dataset.launchMode); show(node.dataset.open); if (node.dataset.side) applySide(node.dataset.side === "sell"); })); $$("[data-nav]").forEach((node) => node.addEventListener("click", () => show(node.dataset.nav))); $$("[data-detail-tab]").forEach((node) => node.addEventListener("click", () => { $$("[data-detail-tab]").forEach((tab) => tab.classList.toggle("active", tab === node)); $$("[data-detail-panel]").forEach((panel) => panel.classList.toggle("active", panel.dataset.detailPanel === node.dataset.detailTab)); })); $$("[data-amount]").forEach((node) => node.addEventListener("click", () => { const input = $("#trade-amount"); if (input) input.value = node.dataset.amount === "MAX" ? "" : node.dataset.amount; invalidateQuote(); })); $$("[data-lang-toggle]").forEach((node) => node.addEventListener("click", () => { const target = pumpLocale() === "zh" ? "en" : "zh"; try { window.localStorage?.setItem(LOCALE_KEY, target); } catch {} document.documentElement.lang = target === "zh" ? "zh-CN" : "en"; text("[data-lang-current]", target === "zh" ? "简体中文" : "English"); toast(target === "zh" ? "语言偏好已保存" : "Language preference saved"); })); };
